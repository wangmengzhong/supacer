...
const Check=function() {
    ...
    me.isChinese = function(p_str) {
        if (!(typeof (p_str) == 'string' || p_str instanceof String)) {
            return false;
        }
        var patrn = /^[\u4E00-\u9FA5\uF900-\uFA2D]+$/;
        return patrn.test(p_str);
    ...
    me.isDateTime = function(p_str) {
        ...
        if (r == null) {
            ...
            if (r.length < 5) {
                ...
                && d.getDate() == r[2] && d.getHours() == r[3] && d
                    .getMinutes() == r[4]);
            } else if (r.length > 5) {
                d = new Date(r[0], r[1] - 1, r[2], r[3], r[4], r[5]);
                return (d.getFullYear() == r[0] && (d.getMonth() + 1) == r[1]
                && d.getDate() == r[2] && d.getHours() == r[3]
                && d.getMinutes() == r[4] && d.getSeconds() == r[5]);
            } else {
                return false;
            }
        }
     ...
     *            一个字符串，表示待打印区域控件的id。
     */
    me.printPageContent = function(p_id) {
        var href=location.href;
        if(p_id){
            var printData = document.getElementById(p_id).innerHTML;//获得div里所有HTML
            var oldBody = document.body.innerHTML;
            var addHtml = "<OBJECT classid='CLSID:8856F961-340A-11D0-A96B-00C04FD705A2' height='0' width='0' id='wb1'></OBJECT>";
            if(!!window.ActiveXObject){//ie浏览器
                document.body.innerHTML = addHtml+printData;//重置整个页面
                wb1.ExecWB(7,1);//打印页面预览
                //wb1.ExecWB(22,1);//刷新
                location.href=href;
            }else{
                document.body.innerHTML = printData;//重置整个页面
                window.print();//打印整个页面（查询列表）
            }
            document.body.innerHTML = oldBody;//还愿
            location.reload();
        }else{
            Rcp.indicate("error", "请指定要打印的区域！");
        }
    };

    /**
     * 将htmlcss属性转化为js可用的属性
     *
     * @param cssAttr, html的css属性。
     */
    me.cssAttrToJSProperty = function(cssAttr){
        //将css标签转换为脚本属性
        cssAttr= cssAttr.toLowerCase();//转为小写，在google下元素style是小写，而IE8是大写
        if(cssAttr){
            var cssAttrs = cssAttr.split('-');
            var jsPorpery = cssAttrs[0];
            for(var j=1; j<cssAttrs.length; j++){
                jsPorpery += cssAttrs[j].substring(0,1).toUpperCase() + cssAttrs[j].substring(1);
            }
            return jsPorpery
        }
    };
    ...
    me.isSpecialChar = function(p_str) {
        if (!(typeof (p_str) == 'string' || p_str instanceof String)) {
            return false;
        }
        var pattern=pattern=/(\……|\￥|\,|\/|\.|\`|~|\@|\&|\*|\?|\%|\<|\>|\(|\)|\-|\_|\^|\!|\;|\"|\:|\||\{|\}|\[|\]|\｛|\｝|\：|\【|\】|#)/i;
        return !pattern.test(p_str);
    };

    return me;
};

const CheckUtil=new Check();
